	...
	<view class="">
        ...
        <view style="height:auto;" @scrolltolower="loadMore()">
            <view v-for="(row,index2) in list" :key="index2">
                <inc-faqs2 :row="row" @close="close()" @click="goRow(row)"></inc-faqs2>
            </view>
        </view>
...
<script>
	...
	import incFaqs2 from '@/components/incs/incFaqs2.vue'
	...
	export default {
		...
		methods: {
            async loadData() {
                ...
                this.status = 'loading';  
                await this.$ureq.commApi('faqs-list', {data:params}).then(res => {
                    if(res.data.ercode){
			...
			goRow(row) {
				uni.navigateTo({
					url: '/pages/faqs/view?did=' + row.did
				});
			},
			loadMore(e) {
